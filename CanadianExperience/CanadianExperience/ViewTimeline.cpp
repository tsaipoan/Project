 ...
 */
 void CViewTimeline::OnTimer(UINT_PTR nIDEvent)
 {
	 ...
	 */
	 ...
	 if (mPlay)
	 {
		 double time = timeline->GetCurrentTime() + elapsed;
		 if (time > timeline->GetDuration())
		 {
			 time = timeline->GetDuration();
			 mPlay = false;
			 GetPicture()->EndSnowFlake();
		 }
 ...
 /**
 * \brief Play the animation in current time
 */
 void CViewTimeline::OnPlayPlay()
 {
	 mPlay = true;
	 GetPicture()->SetPlay(true);
 ...
 /**
 * \brief Play the animation from the begining
 */
 ...
 /**
 * \brief Stop the animation from the current time
 */
