//showing the twitch.tv chat!
	...
	blnScroll = true; //whether or not the page is scrolling
...
function handleChat(channel, user, message, self) {
	...
	//message is the message each person said
	...
	var msg = showEmotes ? formatEmotes(message, user.emotes) : htmlEntities(message);
	chatMessage.innerHTML = msg;

	var myColor = chatMessage.style.color;
	...
	chatButton.addEventListener('click', function() {
		...
		xmlhttp.open("GET", 'http://35.9.22.102/beta/twitch/likeMessage.php?m='+msg+'&c='+String(window.localStorage.getItem("Chans"))+'&u='+name+'&r='+myColor, true);
		xmlhttp.send(null);
	...
	chatLine.appendChild(chatButton); // adding like button
	...
	//pause on hover, resume scrolling when not hover and next message sent
