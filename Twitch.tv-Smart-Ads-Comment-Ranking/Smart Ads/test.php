...
$pop = $dbh->query('SELECT COUNT(ip) FROM Popularity');
$pop_num = 0;
foreach ($pop as $row) {
    $pop_num = $row[0];
}
...
<div class="ad-container">
    <div class="popularity">
        ...
        if (isset($response['Items']['Item']) ) {
            ...
            for ($i = 1; $i <= 15; $i++) {
                if ($i <= $dark_pos) {
                    echo '<img class="light" src="pic/lightOvalSmall.png" style="position: relative; top: -' . 11 * $i . 'px">';
                } else {
                    echo '<img class="light" src="pic/darkOvalSmall.png" style="position: relative; top: -' . 11 * $i . 'px">';
                }
            }
        }
        ?>
    ...
    <?php
    if (isset($response['Items']['Item']) ) {
        echo '<a target="_blank" href="ad.post.php?url=' . $response['Items']['Item'][0]['DetailPageURL'] . '&c=' . $channel . '">';
    }
    ?>
    <div class="ad">
        <div class="pic">
...
<script>
    var key = <?php echo '"'.$search.'"'; ?>;
    var c = <?php echo '"'.$channel.'"'; ?>;
    var clicks = <?php echo '"'.$pop_num.'"'; ?>;
    function refresh() {
        $.post('getKey.php', { channel: c }, function(result) {
            //alert(result);
            //document.write (result);
            $.post('getClick.php', { channel: c }, function(result) {
                //alert(result);
                //document.write (result);
                var clicks2 = String(result);
                if(clicks != clicks2 && Number(clicks) < 15){
